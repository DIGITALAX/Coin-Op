...
import { ALL_FONTS, INFURA_GATEWAY } from "@/app/lib/constants";
import { SynthContext } from "@/app/providers";
import { BottomMenuProps } from "../../types/walkthrough.types";
  ...
  return (
    <div
      className={`relative h-fit inline-flex flex-wrap gap-2 ${
        synthContext?.canvasExpand ? "w-full pb-2 px-4" : "w-1/2 pb-1 px-1"
      }`}
    >
      <div
      ...
      <div
        ...
        onClick={() =>
          !synthLoading &&
          synthContext?.setCanvasExpand(!synthContext?.canvasExpand)
        }
      >
      ...
      {showBottomOptions && (
        <div
          className={`absolute w-full h-fit inline-flex flex-wrap gap-2 ${
            synthContext?.canvasExpand ? "bottom-16 left-14" : "bottom-8 left-9"
          }`}
        >
          <CanvasOption
          ...
          {fontOpen && (
            <div
              className={`absolute w-fit z-1 overflow-y-scroll ${
                synthContext?.canvasExpand ? "bottom-20 h-32" : "bottom-16 h-20"
              }`}
            >
              <div className="relative w-fit h-fit flex flex-col justify-start items-center font-mana text-xxs border border-sol rounded-sm bg-black">
                {ALL_FONTS?.map((value: string, index: number) => {
                  return (
                    <div
          ...
          <div
            ...
            onClick={() => undo(String(synthContext?.current?.layer?.id))}
          >
            <CanvasOption
          ...
          <div
            ...
            onClick={() => redo(String(synthContext?.current?.layer?.id))}
          >
            <CanvasOption
          ...
          {materialOpen && (
            <div
              className={`absolute w-fit h-fit z-1 ${
                synthContext?.canvasExpand ? "bottom-20" : "bottom-16"
              }`}
            >
